...
RET_VOID_NAKE InputLinePatch1_ASM()
{
	__asm {
		mov ebx, eax
		push [edi+8]
...
RET_VOID_NAKE InputLinePatch2_ASM()
{
	__asm {
		call InitD2EditBox
		test eax, eax
...
RET_VOID_NAKE SelectTextPatch1_ASM()
{
	__asm {
		push ecx
		push edx
...
RET_VOID_NAKE SelectTextPatch2_ASM()
{
	__asm {
		push ecx
		push edx
