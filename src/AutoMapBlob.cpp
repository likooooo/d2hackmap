...
RET_VOID_NAKE ForceDrawBlob_ASM()
{
	__asm {
...
RET_VOID_NAKE DrawBlobPath_ASM()
{
//jmp
//esi ==> pUnit
//[esp+4] ==> colour
...
RET_VOID_NAKE DrawPartyPlayerBlobPatch_ASM()
{
	__asm {
//dl = col
//ecx = x pos
...
RET_VOID_NAKE HostilePlayerColor_ASM()
{
	__asm {
		mov ecx, esi
...
RET_VOID_NAKE DrawPlayerTextPatch_ASM()
{
	__asm {
		push ebx
		push edi
...
RET_VOID_NAKE MonsterBlobDescPatch_ASM()
{
	__asm {
//esi = ptr unit
		mov ecx,esi
...
RET_VOID_NAKE MixedBlobColPatch_ASM()
{
	__asm {
//esi = ptr unit
//ebx = ptr colour
